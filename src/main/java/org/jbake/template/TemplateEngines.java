...
/**
 * <p>
 * A singleton class giving access to rendering engines. Rendering engines are loaded based on
 * classpath. New rendering may be registered either at runtime (not recommanded) or by putting a
 * descriptor file on classpath (recommanded).
 * </p>
 * <p/>
 * <p>
 * The descriptor file must be found in <i>META-INF</i> directory and named
 * <i>org.jbake.parser.TemplateEngines.properties</i>. The format of the file is easy:
 * </p>
 * <code>
 * org.jbake.parser.FreeMarkerRenderer=ftl<br> org.jbake.parser.GroovyRenderer=groovy,gsp<br> </code>
 * <p>
 * where the key is the class of the engine (must extend {@link AbstractTemplateEngine} and have a
 * 4-arg constructor and the value is a comma-separated list of file extensions that this engine is
 * capable of proceeding.
 * </p>
 * <p/>
 * <p>
 * Rendering engines are singletons, so are typically used to initialize the underlying template
 * engines.
 * <p/>
...
public class TemplateEngines {
    ...
    public TemplateEngines(final CompositeConfiguration config, final ContentStore db, final File destination,
    ...
    private void registerEngine(String fileExtension, AbstractTemplateEngine templateEngine) {
        ...
        if (old != null) {
            LOGGER.warn("Registered a template engine for extension [.{}] but another one was already defined: {}",
                    fileExtension, old);
        }
    }
     ...
     *            engine class, used both as a hint to find it and to create the engine itself. @return
     *            null if the engine is not available, an instance of the engine otherwise
     */
    private static AbstractTemplateEngine tryLoadEngine(final CompositeConfiguration config, final ContentStore db,
            final File destination, final File templatesPath, String engineClassName) {
        try {
            @SuppressWarnings("unchecked")
            Class<? extends AbstractTemplateEngine> engineClass =
                    (Class<? extends AbstractTemplateEngine>) Class.forName(engineClassName, false,
                            TemplateEngines.class.getClassLoader());
            Constructor<? extends AbstractTemplateEngine> ctor =
                    engineClass
                            .getConstructor(CompositeConfiguration.class, ContentStore.class, File.class, File.class);
     ...
     */
    private void loadEngines(final CompositeConfiguration config, final ContentStore db, final File destination,
            final File templatesPath) {
        try {
    ...
    private void registerEngine(final CompositeConfiguration config, final ContentStore db, final File destination,
            final File templatesPath, String className, String... extensions) {
